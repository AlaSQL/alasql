...
import {FuncValue} from "../yy/FuncValue.js"
import {GroupExpression} from "../yy/GroupExpression.js"
...
export function rollup(a,query) {
	var rr = [];
	var mask = 0;
	var glen = a.length;
...
/**
 GROUPING SETS()
 */
export function groupingsets(a,query) {
 ...
 */
export function decartes(gv,query) {
//	console.log(gv);
...
//		 		res = res.map(function(r){return r.concat(gv[t].columnid)});
			} else if(gv[t] instanceof FuncValue) {
...
//				res = res.concat(gv[t]);
		...
		return res;
	} else if(gv instanceof FuncValue) {
//		console.log(gv);
