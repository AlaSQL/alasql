define(function () {
	...
	SQLite_driver.prototype.getSchemaStructure = function (args) {
        ...
        */
		...
		var schema_sql =
			"select * from sqlite_master where type IN ('table', 'view') and name != '__WebKitDatabaseInfoTable__' order by type,name";
		...
		var localCallback = function (resultSets) {
			...
			for (var j in resultSets[0]['RESULTS']['DATA']) {
				...
				if (tableType == 'table') {
					// sadly, there is no easy way to get the columns back from views in WebSQL
