...
describe('Test 366 wildcards for LIKE', function() {
	var data = [
		...
		describe('with %', function() {
			  ...
			  it.skip('can escape wildcards',function(done){
			  ...
			  it('Finds center',function(done){		// Not supported yet
			  ...
			  it('Finds postpending',function(done){		// Not supported yet
		...
		describe('with ?', function() {
				...
				it('Finds prepending',function(done){
						assert.equal(0,alasql(sql+"'?a'",[data]));
						assert.equal(1,alasql(sql+"'?b'",[data]));
						assert.equal(5,alasql(sql+"'??b'",[data]));
						done();
				});


				it('Finds center',function(done){		// Not supported yet
						assert.equal(1,alasql(sql+"'?a?'",[data]));
						assert.equal(1,alasql(sql+"'?b?'",[data]));
						done();
				});
		...
		describe('with _', function() {
				...
				it.skip('can escape wildcards',function(done){
						assert.equal(1,alasql(sql+"'\\_'",[data]));
						done();
				});
