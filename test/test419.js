...
describe(
	'Test ' + test + ' Load data from text file with default headers option',
	function () {
		before(function () {
		...
		it('1. Load TXT', function (done) {
			alasql(
				'SELECT * FROM TXT("' + __dirname + '/test419a.txt")',
				[],
				function (res) {
		...
		it('3. Load CSV by default', function (done) {
			alasql(
				'SELECT * FROM CSV("' + __dirname + '/test419a.txt")',
				[],
				function (res) {
					assert.deepEqual(res, [
						{words: 'There', letters: 5},
		...
		it('5. Load XLSX', function (done) {
			alasql(
				'SELECT * FROM XLSX("' + __dirname + '/test419.xlsx")',
				[],
				function (res) {
					assert.deepEqual(res, [
						{words: 'don’t', letters: 1},
