...
if (typeof exports === 'object' && false) {
	describe('Test 166 - database in database', function () {
		it('1. Pass-thru database', function (done) {
			...
			res = alasql(
				'select column b from test166(select * from one where a > 2)'
			);
		...
		it('2. Cached sql-statements', function (done) {
			var res = alasql(
				'select a from cache(select * from test166.one where a > 2)'
			);
			...
			res = alasql(
				'select b from cache(select * from test166.one where a > 2)'
			);
			assert.deepEqual(res, [30, 40]);
