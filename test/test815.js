...
(alasql.utils.isNode ? describe : describe.skip)(
	...
	function () {
		...
		it('2. XLSX parses date as number', function () {
			return alasql
				.promise('SELECT * INTO XLSX("test/test815.xlsx") FROM dates')
				.then(function () {
					return alasql
						.promise(
							'SELECT * FROM xlsx("test/test815.xlsx", {cellDates: false})'
						)
						.then(function (res) {
		...
		it('3. XLSX parses date as date', function () {
			return alasql
				...
				.then(function () {
					return alasql
						.promise('SELECT * FROM xlsx("test/test815.xlsx")')
						.then(function (res) {
							...
							assert.equal(
								res[1].date.getTime() - unixepoch.getTime() < 100,
								true
							);
						});
				});
