...
describe('Test 317 GRAPH', () => {
	...
	test.skip('3. Simple graph', done => {
		var res = alasql('SEARCH > "loves" > name FROM #Alex');
		expect(res).toEqual(['Kate', 'Paloma']);
		done();
	});

	test.skip('4. Simple graph', done => {
		var res = alasql('SEARCH / VERTEX AS @p OR(<,>) @p name');
		expect(res).toEqual(['Pablo', 'Maxim', 'Alex', 'Kate', 'Julia', 'Paloma']);
		done();
	});

	test.skip('5. Simple graph', done => {
		var res = alasql('SEARCH / VERTEX AS @p AND(<,>) @p name');
		expect(res).toEqual(['Kate']);
		done();
	});

	test.skip('6. Simple graph', done => {
		var res = alasql('SEARCH / VERTEX AS @p AND(<"loves",<"hates") @p name');
		expect(res).toEqual(['Julia']);
		done();
	});

	test.skip('7. Simple graph', done => {
		var res = alasql('SEARCH DISTINCT(/ VERTEX AS @p < OR("loves","hates") @p name)');
		expect(res).toEqual(['Kate', 'Julia', 'Paloma']);
