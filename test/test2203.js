...
describe('Test 2203 - KEY keyword should be usable as identifier', function () {
	...
	it('A) KEY as column name in CREATE TABLE', function () {
		var res = alasql('CREATE TABLE test_key_col (id INT, key VARCHAR(50), value VARCHAR(100))');
		assert.equal(res, 1);
		...
		assert.deepEqual(selectRes, [{id: 1, key: 'mykey', value: 'myvalue'}]);
	});
	...
	it('E) SELECT with KEY column in WHERE clause', function () {
		alasql('CREATE TABLE items (id INT, key VARCHAR(50), value VARCHAR(100))');
		alasql("INSERT INTO items VALUES (1, 'key1', 'value1'), (2, 'key2', 'value2')");
		
		var res = alasql("SELECT * FROM items WHERE key = 'key1'");
		assert.deepEqual(res, [{id: 1, key: 'key1', value: 'value1'}]);
	});
	...
	it('I) INDEX KEY syntax still works', function () {
		var res = alasql('CREATE TABLE test_index (id INT, name VARCHAR(50), KEY idx_name (name))');
		assert.equal(res, 1);
	});
	...
	it('L) GROUP BY with KEY column', function () {
		alasql('CREATE TABLE test_group (key VARCHAR(50), value INT)');
		alasql("INSERT INTO test_group VALUES ('a', 1), ('b', 2), ('a', 3)");
		
		var res = alasql('SELECT key, SUM(value) as total FROM test_group GROUP BY key ORDER BY key');
