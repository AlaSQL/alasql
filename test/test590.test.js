...
describe('Test 590 - LEFT() and RIGHT() SQL string functions', function () {
	...
	test('6. RIGHT() with edge cases', function () {
		// Empty string
	...
	test('7. LEFT() and RIGHT() in SELECT list', function () {
		var data = [{text: 'AlaSQL'}];
		var res = alasql('SELECT LEFT(text, 3) as left_part, RIGHT(text, 3) as right_part FROM ?', [
			data,
		]);
		expect(res[0].left_part).toBe('Ala');
		expect(res[0].right_part).toBe('SQL');
	});

	test('8. LEFT() and RIGHT() with WHERE clause', function () {
		var data = [
			{id: 1, name: 'Alpha'},
			{id: 2, name: 'Beta'},
			{id: 3, name: 'Gamma'},
	...
	test('14. LEFT/RIGHT with UPPER/LOWER functions', function () {
		var data = [{name: 'JavaScript'}];
		var res = alasql(
			'SELECT UPPER(LEFT(name, 4)) as upper_left, LOWER(RIGHT(name, 6)) as lower_right FROM ?',
			[data]
		);
		expect(res[0].upper_left).toBe('JAVA');
		expect(res[0].lower_right).toBe('script');
	});

	test('15. LEFT/RIGHT with CONCAT', function () {
		var res = alasql('SELECT VALUE CONCAT(LEFT("Hello", 3), RIGHT("World", 3))');
		expect(res).toBe('Helrld');
	});

	test('16. LEFT/RIGHT with special characters', function () {
		...
		expect(res2).toBe('domain.com');
	});

	test('17. User-defined function compatibility', function () {
