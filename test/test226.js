...
describe('Test 226 CROSS APPLY and OUTER APPLY', function() {
    ...
    it("1. CROSS APPLY", function(done) {
    	var one = [{a:1},{a:2},{a:3}];
    	var two = [{a:1,b:10},{a:2,b:20},{a:3,b:30}];
    	var res = alasql('SELECT one.a,three.b FROM ? one \
    		CROSS APPLY \
    		(SELECT b FROM ? two WHERE one.a = two.a) three',[one,two]);
    	assert.deepEqual(res,[ { a: 1, b: 10 }, { a: 2, b: 20 }, { a: 3, b: 30 } ]);
		done();
    });

    it("2. CROSS APPLY", function(done) {
    	var one = [{a:1},{a:2},{a:3}];
    	var two = [{a:1,b:10},{a:2,b:20},{a:2,b:30},{a:4,b:40}];
    	var res = alasql('SELECT one.a,three.b FROM ? one \
    		CROSS APPLY \
    		(SELECT b FROM ? two WHERE one.a = two.a) three',[one,two]);
//    	console.log(res);
    	assert.deepEqual(res,[ { a: 1, b: 10 }, { a: 2, b: 20 }, { a: 2, b: 30 } ]);
		done();
    });
