...
describe(
	'Test ' + test + ' - non-numeric values for SUM, MIN and MAX',
	function () {
		it('MAX dealing with non-numeric values', function () {
			...
			assert.deepEqual(res, [
				{a: null, b: 9, c: null, c2: 1, d: 5, e: null, f: 11},
			]);
			var data = [
				{
					a: null,
		...
		it('MIN dealing with non-numeric values', function () {
			...
			assert.deepEqual(res, [
				{a: null, b: 1, c: null, c2: 1, d: 5, e: null, f: 2},
			]);
			var data = [
				{
					a: null,
		...
		it('SUM dealing with non-numeric values', function () {
			...
			assert.deepEqual(res, [
				{a: null, b: 10, c: null, c2: 1, d: 5, e: null, f: 13},
			]);
		...
		it('TOTAL dealing with non-numeric values', function () {
			...
			assert.deepEqual(res, [
				{a: 0, b: 10, c: 1, c2: 1, d: 5, e: 0, f: 13, g: -1, h: 1},
			]);
		});

		it('TOTAL of nothing is zero', function () {
			data = [{ProductId: 10, price: 50}];
			res = alasql(`SELECT TOTAL(price) AS p FROM ? WHERE ProductId = 5`, [
				data,
			]);
		...
		it('TOTAL of two rows based on select', function () {
			...
			res = alasql(`SELECT TOTAL(price) AS p FROM ? WHERE ProductId = 100`, [
				data,
			]);
