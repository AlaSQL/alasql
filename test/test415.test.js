...
describe('Test ' + testNum + ' Aggregators', () => {
	beforeAll(() => {
	...
	test('1. Test', done => {
		...
		expect(res).toEqual([{medparam: 5000}]);
	...
	test('2. Test', () => {
		...
		expect(res).toEqual([{'MEDIAN(a)': 2, 'STDEV(a)': 1, 'SQRT(VAR(a))': 1}]);
	});
	...
	test('4. Quatiles', () => {
		...
		expect(res).toEqual([{'QUART(a)': 4, 'QUART2(a)': 7, 'QUART3(a)': 10}]);
	});

	test('5. GREATEST/LEAST', () => {
		var res = alasql(
			'SELECT LEAST(3, 12, 34, 8, 25) AS numL, GREATEST(3, 12, 34, 8, 25) AS numG, LEAST("w3", "mmco", "a") AS strL, GREATEST("w3", "mmco", "a") AS strG'
		);
		expect(res).toEqual([{numL: 3, numG: 34, strL: 'a', strG: 'w3'}]);
