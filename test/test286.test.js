...
describe('Test 286 CREATE UNIQUE INDEX', () => {
	...
	test('1. CREATE TABLE and UNIQUE INDEX', () => {
		...
		alasql('CREATE TABLE test286.users (id INT PRIMARY KEY, email STRING, name STRING)');
		alasql('CREATE INDEX idx_users_email ON test286.users(email)');
	});

	test('2. INSERT unique values - should succeed', () => {
		var res = alasql('INSERT INTO test286.users VALUES (1, "user1@test.com", "User One")');
		expect(res).toBe(1);

		res = alasql('INSERT INTO test286.users VALUES (2, "user2@test.com", "User Two")');
		expect(res).toBe(1);

		res = alasql('INSERT INTO test286.users VALUES (3, "user3@test.com", "User Three")');
		expect(res).toBe(1);
	});

	test('3. INSERT duplicate primary key - should fail', () => {
		expect(() => {
			alasql('INSERT INTO test286.users VALUES (1, "user4@test.com", "User Four")');
		}).toThrow(Error);
	});
	...
	test('4. INSERT duplicate email with index - currently allowed (UNIQUE INDEX not enforced)', () => {
		...
		var res = alasql('INSERT INTO test286.users VALUES (4, "user1@test.com", "User Four")');
		expect(res).toBe(1);
	});
	...
	test('5. Verify index creation and basic functionality', () => {
		...
		res = alasql('SELECT * FROM test286.users ORDER BY id');
