...
describe('Test 2413 - GROUP_CONCAT with DISTINCT, ORDER BY and SEPARATOR', function () {
	...
	it('B) GROUP_CONCAT with DISTINCT', function () {
		...
		assert.ok(res[0].countries.includes('Canada'));
		assert.ok(res[0].countries.includes('Mexico'));
		// Count commas - should be 2 (3 unique values)
		assert.equal((res[0].countries.match(/,/g) || []).length, 2);
	});
	...
	it('H) GROUP_CONCAT with GROUP BY clause', function () {
		...
		assert.equal(res[0].region, 'EU');
		assert.equal(res[0].countries, 'France;Germany');
		assert.equal(res[1].region, 'NA');
		assert.equal(res[1].countries, 'Canada;Mexico;USA');
	});
});
