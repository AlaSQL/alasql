...
describe('Test '+test+' - INTO CSV', function() {
	...
	it('With quote = \'\', single multiword string value', function() {
		return alasql.promise("SELECT 'swing out' AS `colname` INTO CSV('test612-1', {quote:''})")
			.then(function(state) {
	...
	it('With quote = \'\', multiple rows', function() {
		return alasql.promise("SELECT a, b INTO CSV('test612-2', {quote:''}) FROM one")
			.then(function(state) {
	...
	it('With quote = \'\\?\', single multiword string value', function() {
		return alasql.promise("SELECT 'swing out' AS `colname` INTO CSV('test612-3', {quote:'?'})")
			.then(function(state) {
