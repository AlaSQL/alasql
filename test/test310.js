...
describe('Test 310 Create graph', function() {
  ...
  it('1. CREATE GRAPH',function(done){
    alasql('CREATE GRAPH #Andrey');
    alasql('CREATE GRAPH #John,#Mary');
    alasql('CREATE GRAPH #Anton,#Julia,#Anton >> #John');
    alasql('CREATE GRAPH #Victor "Victor Branson"');
    alasql('CREATE GRAPH #[John Smith] {age:23, country:"Canada"}');
    alasql('CREATE GRAPH #[John Smith] > "loves" > #Mary');
    alasql('CREATE GRAPH #Anton > "loves" {power:"too much"} > #Julia');
