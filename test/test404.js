...
describe('Test 404 OUTER JOIN', function() {
  ...
  var data = {
    ...
    var res = alasql('select t0.name t0n ,t1.name t1n, t2.name t2n from ? t0 outer join ? t1 on t1.name = t0.name outer join ? t2 on t2.name = t0.name or t2.name = t1.name',[data.COLORS,data.FRUITS, data.MASCOTS]);
    console.log(res);
    done();
  });
