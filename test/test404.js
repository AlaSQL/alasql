...
(2, 'blue'), \
(3, 'orange')");
    alasql("insert into fruits(id, name) values \
(1, 'apple'), \
(2, 'grape'), \
(3, 'orange'), \
(4, 'peaches')");
    alasql("insert into mascots(id, name) values \
(1, 'redsox'), \
(2, 'whitesox'), \
(3, 'orange'), \
(4, 'peaches')");
  ...
  it('3. Test FULL OUTER JOIN with 2 tables', function(done) {    
    var res = alasql('select t0.name t0n ,t1.name t1n from colors t0 full outer join fruits t1 on t1.name = t0.name');
  ...
  it('4. Test FULL OUTER JOIN with 3 tables using cte workaround', function(done) {
    var res = alasql('with t1 as (select COALESCE(t0.name, t1.name) AS name, t0.name as t0n, t0.id as t0id, t1.name as t1n, t1.id as t1id FROM colors t0 full outer join fruits t1 on t1.name = t0.name) select t0n, t1n, t2.name as t2n from t1 full outer join mascots t2 on t2.name = t1.name');
