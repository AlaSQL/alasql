...
describe('Test 306 XML reader', () => {
	...
	test('1. Read XML file / SEARCH like JSON', done => {
		alasql(
			'SEARCH children/"Worksheet" attributes [ss:Name] FROM XML("' + __dirname + '/test306.xml")',
			[],
			function (res) {
				expect(res).toEqual(['Sheet1', 'demo']);
				done();
			}
		);
	});

	test('1a. Read XML file / SEARCH XML', done => {
	...
	test('2. Read XML file / SEARCH XML', done => {
		//    alasql('SEARCH xml /Worksheet%[ss:Name] FROM XML("test306.xml")',[],function(res){
		alasql(
			'SEARCH XML Worksheet %[ss:Name] FROM XML("' + __dirname + '/test306.xml")',
			[],
	...
	test('3. Read XML file / SEARCH XML', done => {
		alasql('SEARCH XML / * Data$ FROM XML("' + __dirname + '/test306.xml")', [], function (res) {
			//       console.log(res);
			expect(res).toEqual(['aaaa', '2', '3', '5', '6', '7']);
			done();
		});
	});

	test('4. Read XML file / SEARCH XML', done => {
		alasql(
	...
	test('5. Read XML file / SEARCH XML', done => {
		alasql('SEARCH xml %xmlns FROM XML("' + __dirname + '/test306.xml")', [], function (res) {
			//      console.log(res);
			expect(res).toEqual(['urn:schemas-microsoft-com:office:spreadsheet']);
			done();
		});
	});

	test('6a. Read GEFX file / SEARCH XML', done => {
	...
	test('6b. Read GEFX file / SEARCH XML', done => {
		//     alasql('SEARCH XML /graph/nodes/% {[$id]:id,name:label} FROM XML("test306a.xml")',[],function(res){
		alasql(
	...
	test('7. Edges ', done => {
		...
		alasql(
			...
			function (res) {
				//        console.log(res);
				expect(res).toEqual([{id: '0', source: '0', target: '1'}]);
				done();
			}
		);
	});

	test('7. SEARCH INTO ', done => {
		alasql(
			...
			function (res) {
				...
				//        console.log('>>',res,'<<');
				expect(res).toEqual('"id";"source";"target"\r\n0;0;1\r\n');
				//         expect(res).toEqual([ { id: '0', source: '0', target: '1' } ]);
				done();
			}
		);
