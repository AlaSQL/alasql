...
describe('Test 277 NVARCHAR(precision) - issue #150', () => {
	test('1. Prepare databases', done => {
		alasql('CREATE DATABASE test277');
		done();
	});

	test('2. INFORMATION_SCHEMA', done => {
		alasql('CREATE TABLE test277.one (a INT, b NVARCHAR(10), c CHAR(12), d FIXED(1,2))');
		alasql(
			'INSERT INTO test277.one VALUES (1,"One","one",1.1), (2,"Two","two",1.2), (3,"Three","three",1.3)'
		);
		var res = alasql('SELECT RECORDSET * FROM test277.one');
