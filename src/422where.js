yy.Select.prototype.compileWhere = function (query) {
	if (this.where) {
		if (typeof this.where == 'function') {
			...
			query.wherefns = s;
			//		console.log(s);
			return new sandboxed_function('p,params,alasql', 'var y;return ' + s);
		}
	} else
		return function () {
			return true;
...
yy.Select.prototype.compileWhereJoins = function (query) {
	...
	query.sources.forEach(function (source) {
		if (source.srcwherefns) {
			source.srcwherefn = new sandboxed_function('p,params,alasql', 'var y;return ' + source.srcwherefns);
		}
		if (source.wxleftfns) {
			source.wxleftfn = new sandboxed_function('p,params,alasql', 'var y;return ' + source.wxleftfns);
		}
		if (source.wxrightfns) {
			source.wxrightfn = new sandboxed_function('p,params,alasql', 'var y;return ' + source.wxrightfns);
		}
