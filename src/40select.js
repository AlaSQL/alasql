...
function modify(query, res) {
	...
	switch (modifier) {
		...
		case 'RECORDSET': {
			...
			if (columns) columns.forEach(c => allKeys.add(c.columnid));
			if (res && res.length > 0)
				Object.keys(res[0]).forEach(k => allKeys.add(k));
			const mergedColumns = Array.from(allKeys).map(k => ({ columnid: k }));
			return new alasql.Recordset({ columns: mergedColumns, data: res });
		}
