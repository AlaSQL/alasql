...
//		} 
		...
		if(self.joins[k].type == 'LEFT' && !pass) {
			scope[self.joins[k].tableid] = {};
			self.doJoin(scope,k+1);
		}	
	}
}
...
nodes.Select.prototype.exec = function (db) {
	...
	if(this.joins.length > 0) {
		query.joins = this.joins.map(function(join){ 
			return join.compile(tableid); 
		}).map(function(join){
			join.recs = db.tables[join.tableid];
			return join;
		});
	} else {
		query.joins = [];
	...
	if(this.group) query.groupfn = this.group.compile();
...
// GROUP BY
nodes.Group.prototype.compile = function (tableid) {
	var s = '';
	...
	s += this
		.fields.filter(function(f){
			return (f.field.constructor.name == 'FunctionValue' 
				&& (f.field.name.toUpperCase() == 'SUM' || f.field.name.toUpperCase() == 'COUNT'))
		}).map(function(f){
			return 'group.'+f.name.value+'=(group.'+f.name.value+'||0)+rec.'+f.name.value+';'; //f.field.arguments[0].toJavaScript(); 
		}).join('');
//	s += '}';
