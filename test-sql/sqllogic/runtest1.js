...
var alasql = require('../alasql/dist/alasql.js');
var fs = require('fs');
...
//			console.log(131,qtype);
			if(qtype.length > 3) { // We skip queries with joining more than 3 tables
				continue;
			}
...
//		console.log('ooookkkk2');
			if(success) {
				...
				// Check results
				var passed;
				if(res.length == explen) { // Check if length are the same

//				console.log('IK', explen, res.length,exphash);
					if(exphash) { // Special case
					    var rs = res.map(function(d){return d[rs.columns[0].columnid]+'\n'}).join('');
    					rhash = md5(rs);
//    					console.log(153,rhash,exphash);
